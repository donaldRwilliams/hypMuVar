...
#'
#' @examples
muvar <- function(y, cluster, predictor, data,
                  mixture = FALSE, type = NULL, k = 2, tau_sd = NULL,
                  tau_sd_df = NULL, z_slab_sd = NULL,
                  z_spike_sd = NULL, pr_prob_rho = NULL, chains = 4,
  ...
  # check cluster
  check_cluster <- eval(arg[[match("cluster", names(arg))]], envir = data)
  ...
  # cluster
  ID <- as.numeric(eval(arg[[match("cluster", names(arg))]], envir = data))
